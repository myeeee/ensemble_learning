...
class DecisionStump:
    ...
    def __init__(self, classifier, metrix=infgain):
        self.left = copy.deepcopy(classifier)
        self.right = copy.deepcopy(classifier)
        self.metrix = metrix
        self.feat = None
        self.val = None
...
if __name__ == '__main__':
    ...
    # reading dataset
    #data_airfoil = pd.read_table('dataset/airfoil_self_noise.dat', header=None, names=['Frequency', 'AttackAngle', 'ChordLength', 'StreamVelocity', 'DisplacementThickness', 'PressureLevel'])
    data_iris = pd.read_csv('dataset/iris.data', header=None, names=['SepalLength', 'SepalWidth', 'PetalLength', 'PetalWidth', 'Class'])

    # convert label into value
    ...
    model = DecisionTree(ZeroRule(), metrix=infgain, max_depth=3)
    model.fit(train_X, prob_Y)

    # prediction
    train_Z = model.predict(train_X)
    train_Z = np.argmax(train_Z, axis=1)
    acc_train = np.count_nonzero(train_Y == train_Z) / train_Z.shape[0]
    print(acc_train)

    # prediction
    test_Z = model.predict(test_X)
    test_Z = np.argmax(test_Z, axis=1)
    acc_test = np.count_nonzero(test_Y == test_Z) / test_Z.shape[0]
    print(acc_test)
